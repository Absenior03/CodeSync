  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (gradient && Array.isArray(gradient) && gradient.length >= 2) {
        ...
        gradient.forEach((stopColor, index) => {
          grad.addColorStop(index / (gradient.length - 1), stopColor);
        });
      ...
      const run = (timestamp) => {
        ...
        for (let row = 0; row < tightHeight; row += 1) {
          let dx = 0;
          let dy = 0;
          ...
          ctx.drawImage(offscreen, 0, row, offscreenWidth, 1, dx, row + dy, offscreenWidth, 1);
        }
